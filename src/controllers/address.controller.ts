...
export const saveLocationAndGetShipping = async (req: Request, res: Response) => {
  try {
    ...
    if (await isWarehouseEnabled()) {
      ...
      const result = await calculateShippingWithConfig(
        latitude, longitude, data.country ?? "", data.state ?? "", data.zipcode ?? "", config, warehouse.lat, warehouse.lng,
      );
...
export const saveManualAddress = async (req: Request, res: Response) => {
  try {
    ...
    if (await isWarehouseEnabled()) {
      const [warehouse, config] = await Promise.all([getWarehouseCoords(), getShippingConfigFromDB()]);
      shippingCharge = (await calculateShippingWithConfig(0, 0, country, state ?? "", zipCode, config, warehouse.lat, warehouse.lng)).shippingCharge;
    }
...
export const previewShipping = async (req: Request, res: Response) => {
  try {
    ...
    // Manual mode — no GPS, use provided state/zipCode for rate lookup
    if (manual) {
      const result = await calculateShippingWithConfig(0, 0, "India", manualState ?? "", req.body.zipCode ?? "", config, warehouse.lat, warehouse.lng);
    ...
    const result = await calculateShippingWithConfig(latitude, longitude, country, state, zipCode, config, warehouse.lat, warehouse.lng);
