...
const KEYS = ["COMPANY_NAME", "COMPANY_TAGLINE", "COMPANY_LOGO", "COMPANY_FAVICON", "SHOW_COMPANY_NAME", "SHOW_COMPANY_TAGLINE", "ANNOUNCEMENT_BAR", "ANNOUNCEMENT_BAR_ENABLED"] as const;
...
const DEFAULT_HIGHLIGHTS_CONFIG = {
  enabled: true,
  sectionTitle: "WHY CHOOSE US",
  sectionSubtitle: "Crafted with Purity & Passion",
  items: [
    {
      id: "1",
      title: "NON ALCOHOLIC ATTAR",
      subtitle: "100% Pure & Alcohol Free",
      icon: "leaf",
      image: "",
    },
    {
      id: "2",
      title: "100% PURE FRAGRANCE OIL",
      subtitle: "Unmatched Purity",
      icon: "flask",
      image: "",
    },
    {
      ...
      subtitle: "Handpicked Luxury Ingredients",
      icon: "diamond",
      image: "",
    },
    {
      id: "4",
      title: "LONG LASTING & SKIN FRIENDLY",
      subtitle: "Dermatologically Safe & Lasts All Day",
      icon: "clock",
      image: "",
    },
    {
      ...
      subtitle: "Cruelty Free & Eco Friendly",
      icon: "shield",
      image: "",
    },
  ],
...
const DEFAULT_ABOUT_CONFIG = {
  ...
  description: "Our luxury non-alcoholic attars are meticulously crafted using the finest ingredients, blending tradition with modern perfection.",
  ctaText: "KNOW MORE",
  ctaLink: "/products",
...
// GET /api/admin/homepage-config  — admin only (hero + footer + highlights + about template config)
export const getHomepageConfig = async (_req: Request, res: Response) => {
  try {
    const rows = (await prisma.appSetting.findMany({
      where: { key: { in: ["HERO_CONFIG", "FOOTER_CONFIG", "HIGHLIGHTS_CONFIG", "ABOUT_CONFIG"] } },
    })) || [];
    ...
    const highlightsConfig = parseJsonOrDefault(map["HIGHLIGHTS_CONFIG"], DEFAULT_HIGHLIGHTS_CONFIG);
    const aboutConfig = parseJsonOrDefault(map["ABOUT_CONFIG"], DEFAULT_ABOUT_CONFIG);

    res.status(200).json({ heroConfig, footerConfig, highlightsConfig, aboutConfig });
  } catch (err: any) {
...
// PUT /api/admin/homepage-config/highlights  — admin only
export const updateHighlightsConfig = async (req: Request, res: Response) => {
  try {
    const config = req.body as object;
    const value = JSON.stringify(config);
    await prisma.appSetting.upsert({
      where: { key: "HIGHLIGHTS_CONFIG" },
      update: { value },
      create: { key: "HIGHLIGHTS_CONFIG", value },
    });
    res.status(200).json({ message: "Highlights config updated", highlightsConfig: config });
  } catch (err: any) {
    logger.error("updateHighlightsConfig error", err);
    res.status(500).json({ message: "Error updating highlights config" });
  }
};

// PUT /api/admin/homepage-config/about  — admin only
export const updateAboutConfig = async (req: Request, res: Response) => {
  try {
    const config = req.body as object;
    const value = JSON.stringify(config);
    await prisma.appSetting.upsert({
      ...
      update: { value },
      create: { key: "ABOUT_CONFIG", value },
    });
    res.status(200).json({ message: "About section config updated", aboutConfig: config });
  } catch (err: any) {
    logger.error("updateAboutConfig error", err);
    res.status(500).json({ message: "Error updating about section config" });
  }
};
